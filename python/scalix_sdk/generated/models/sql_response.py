from __future__ import annotations

from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

if TYPE_CHECKING:
    from ..models.sql_response_rows import SqlResponseRows


T = TypeVar("T", bound="SqlResponse")


@_attrs_define
class SqlResponse:
    """Result set returned by a SQL query.

    Attributes:
        columns (list[str]): Column names, in result order.
        rows (SqlResponseRows): Rows; each row is an array of cell values aligned to `columns`.
        row_count (int): Number of rows returned (or affected).
        duration_ms (float): Server-side execution time in milliseconds.
    """

    columns: list[str]
    rows: SqlResponseRows
    row_count: int
    duration_ms: float
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        columns = self.columns

        rows = self.rows.to_dict()

        row_count = self.row_count

        duration_ms = self.duration_ms

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "columns": columns,
                "rows": rows,
                "row_count": row_count,
                "duration_ms": duration_ms,
            }
        )

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.sql_response_rows import SqlResponseRows

        d = dict(src_dict)
        columns = cast("list[str]", d.pop("columns"))

        rows = SqlResponseRows.from_dict(d.pop("rows"))

        row_count = d.pop("row_count")

        duration_ms = d.pop("duration_ms")

        sql_response = cls(
            columns=columns,
            rows=rows,
            row_count=row_count,
            duration_ms=duration_ms,
        )

        sql_response.additional_properties = d
        return sql_response

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
